...
const BASE = (req, res) => {
    const cancelCheckout = async () => {
        ...
        await cancelPool.request()
            ...
            .query(`DELETE FROM Office.Reservations WHERE ReservationCode = @officeCode`)
        pool.close()
        return true
    }
...
const CANCEL = (req, res) => {
    const getOfficeInfo = async () => {
        ...
        let data = await officePool.request()
            ...
            .query(`SELECT R.ReservationEmployee, R.ReservationCode, O.OfficeName 
            FROM Office.Reservations R
                INNER JOIN Office.Offices O ON O.OfficeIndex = R.ReservationOffice
            WHERE ReservationIndex = @checkoutID`)
    ...
    getOfficeInfo()
        .then(resultOffice => {
            ...
            transporter.sendMail({
                ...
                to: resultOffice.ReservationEmployee,
                subject: `${resultOffice.OfficeName} Cancelation Code`,
                html: `<p>Please use the following code to cancel your selected office checkout.</p><br><p><strong>${resultOffice.ReservationCode}</strong></p>`
            })
                // })
                // .then(() => {
